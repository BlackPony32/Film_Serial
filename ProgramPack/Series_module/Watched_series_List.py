...
class _Watched_Series_List(MyWindowFormat):
    def __init__(self):
        super().__init__()

        self.setWindowTitle("Список переглянутих серіалів")
    ...
    def initialize(self):
        ...
        self.table_widget.setFixedSize(2500, 500)
    ...
    def connect_to_postgresql(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM  Watched_Series_list")
            rows = cursor.fetchall()
            # Приховання стовпця з нумерацією
            self.table_widget.verticalHeader().setVisible(False)
            self.table_widget.setRowCount(len(rows))
            desired_column_count = 11
    ...
    def update_database(self):
        try:
            ...
            for row in range(self.table_widget.rowCount()):
                ...
                series_status = self.table_widget.item(row, 4).text()
                series_directors = self.table_widget.item(row, 5).text()
                series_actors = self.table_widget.item(row, 6).text()
                series_genre = self.table_widget.item(row, 7).text()
                series_mark = self.table_widget.item(row, 8).text()
                age = self.table_widget.item(row, 9).text()
                series_description = self.table_widget.item(row, 10).text()
                ...
                cursor.execute(
                    "UPDATE Watched_Seriesr_list SET series_name = %s, date_added = %s, season_quantity = %s, series_status = %s, "
                    "series_directors = %s, series_actors = %s, series_genre = %s,series_mark = %s, age = %s, series_description = %s WHERE unique_id = %s",
                    (series_name, date_added, season_quantity, series_status, series_directors, series_actors, series_genre, series_mark, age, series_description, unique_id)
                )
            conn.commit()
            cursor.close()
            conn.close()
    ...
    def delete_movie(self, unique_id):
        try:
            ...
            cursor.execute(
                "DELETE FROM Watched_Seriesr_list WHERE unique_id = %s",
                (unique_id,)
            )
            conn.commit()
            cursor.close()
