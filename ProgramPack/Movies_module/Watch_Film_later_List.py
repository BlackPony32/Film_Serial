...
class _Watch_Film_LaterList(MyWindowFormat):
    def __init__(self):
        super().__init__()

        self.setWindowTitle("Список фільмів на потім")
    ...
    def connect_to_postgresql(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT unique_id, title, release_date, description FROM Movie_later_list")
            rows = cursor.fetchall()
            # Приховання стовпця з нумерацією
            self.table_widget.verticalHeader().setVisible(False)
            self.table_widget.setRowCount(len(rows))
            desired_column_count = 4
            ...
            header_labels = ["ID", "Назва фільма", "Дата додавання", "Опис фільма"]
            self.table_widget.setHorizontalHeaderLabels(header_labels)

            header = self.table_widget.horizontalHeader()
            column_widths = [50, 200, 130, 620]  # Встановіть бажані довжини для кожного стовпця
    ...
    def update_database(self):
        try:
            ...
            for row in range(self.table_widget.rowCount()):
                ...
                movie_description = self.table_widget.item(row, 3).text()
                ...
                cursor.execute(
                    "UPDATE Movie_later_list SET title = %s, release_date = %s, description = %s WHERE unique_id = %s",
                    (movie_name, date_added, movie_description, unique_id)
                )
            conn.commit()
            cursor.close()
            conn.close()
    ...
    def delete_movie(self, unique_id):
        try:
            ...
            cursor.execute(
                "DELETE FROM Movie_later_list WHERE unique_id = %s",
                (unique_id,)
            )
            conn.commit()
            cursor.close()
